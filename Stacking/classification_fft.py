...
@software:PyCharm
@file: classification_fft
@time: 2019/3/26 16:34
@desc:
'''
...
def onehot(label):
    ...
    '''
    # print(Counter(label))
    one_hot = np.zeros(shape= (label.shape[0], 6))
...
def train(training_time, is_finishing):
    ...
    with NN_graph.as_default():
        ...
        with tf.name_scope('loss-potimize-evaluation-acc'):
            ...
            loss_fnn = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=fnn_op, labels=y))
            #添加摘要loss
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options), graph=NN_graph) as sess:
        ...
        # 导入数据
        p_dataset_ori = r'F:\ProximityDetection\Stacking\dataset_PNY\PNY_fft_norm_c.pickle'
        # 记录折数
        fold = 0
        # 总训练轮数
        epoch_all = 10000
        ...
        for train, test in data_stepone_1(p_dataset_ori=p_dataset_ori, proportion=5, is_shuffle=True):
            ...
            test_ont_hot = onehot(test[:, -1]) #(2000, 6)
                ...
                for batch_x, batch_y in data_steptwo(train_data=train, batch_size=1000):
                    ...
                    if (epoch % 100) == 0 and flag == 1:
                        loss_fnn_ = sess.run(loss_fnn, feed_dict={x: batch_x[:, :100], x_add: batch_x[:, 100:],
                                                                  y: batch_y,
                                                                  is_training: False})
                        acc_fnn_ = sess.run(acc_fnn, feed_dict={x: test[:, :100], x_add: test[:, 100:-1],
                                                                y: test_ont_hot,
                                                                is_training: False})
