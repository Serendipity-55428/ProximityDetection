...
def data_make():
    ...
    PNY_data = pd.DataFrame(PNY_data, columns=[i for i in range(1, PNY_data.shape[-1] + 1)])
    ...
    SaveFile(data=PNY_data_classifier, savepickle_p=r'F:\ProximityDetection\Stacking\dataset_PNY\PNY_norm_cl.pickle')
...
def batch_normoalization(input, is_training, moving_decay= 0.9, eps= 1e-5):
    ...
    #训练时，更新均值与方差，测试时使用之前各个层所有mini-batch的保存的均值与方差的期望
...
def network(x_p, x_f, is_training):
    ...
    with tf.name_scope('cnn'):
        ...
        conv2 = tf.keras.layers.Conv2D(filters=128, kernel_size=[2, 2], padding='same', activation=tf.nn.relu,
                                       kernel_initializer=tf.keras.initializers.TruncatedNormal,
                                       name='conv2')(bn1) #(None, 2, 3, :)
        conv3 = tf.keras.layers.Conv2D(filters=128, kernel_size=[2, 2], padding='same', activation=tf.nn.relu,
                                       kernel_initializer=tf.keras.initializers.TruncatedNormal,
                                       name='conv3')(conv2) #(None, 2, 3, :)
        conv4 = tf.keras.layers.Conv2D(filters=256, kernel_size=[1, 2], padding='same', activation=tf.nn.relu,
                                       kernel_initializer=tf.keras.initializers.TruncatedNormal,
                                       name='conv4')(conv3) #(None, 2, 3, :)
        flat = tf.keras.layers.Flatten(name='flat')(conv4) #(None, 2*3*:)
        # print(flat)

    with tf.name_scope('rnn'):
        input_rnn = tf.reshape(tensor=flat, shape=[-1, 32, 48], name='input_rnn')
...
def sess(training_time, is_finishing):
    ...
    with g.as_default():
        ...
        with tf.name_scope('placeholder'):
            x_p = tf.placeholder(shape=[None, 20], dtype=tf.float32, name='x_p')
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options), graph=g) as sess:
        ...
        #导入数据
        p=r'F:\ProximityDetection\Stacking\dataset_PNY\PNY_norm_cl.pickle'
        # 记录折数
        fold = 0
        # 总训练轮数
        epoch_all = 1000
        ...
        for train, test in data_stepone_1(p_dataset_ori=p, proportion=4, is_shuffle=True):
            ...
            for epoch in range(epoch_all):
                ...
                for batch_x, batch_y in data_steptwo(train_data=train, batch_size=5*190):
                    ...
                    summary = sess.run(merge, feed_dict={x_p: batch_x[:, 4:], x_f: batch_x[:, :4],
                                                         y: batch_y, learning_rate: LEARNING_RATE,
                                                         is_training: True})
                    _ = sess.run(opt, feed_dict={x_p: batch_x[:, 4:], x_f: batch_x[:, :4],
                                                          y: batch_y, learning_rate: LEARNING_RATE,
                                                          is_training: True})
                    ...
                    if (epoch % 100) == 0 and flag == 1:
                        loss_, train_acc = sess.run([loss, acc], feed_dict={x_p: batch_x[:, 4:], x_f: batch_x[:, :4],
                                                                             y: batch_y, is_training:False,
                                                                             learning_rate:LEARNING_RATE})

                        test_acc = sess.run(acc, feed_dict={x_p:test[:, 4:-4], x_f:test[:, :4], y:test[:, -4:],
                                                             is_training:False})
        ...
        if is_finishing:
            # 将最终训练好的模型保存为pb文件
            savemodel = SaveImport_model(sess_ori=sess, file_suffix='\\cl_model', ops=(output, x_p, x_f),
