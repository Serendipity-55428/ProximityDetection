...
def cnn(x, is_training, summary_visualization):
    ...
    with tf.name_scope('cnn'):
        ...
        pool1 = cnn_1.pooling(pool_fun=tf.nn.max_pool, input=bn1) #(-1, 5, 5, 32)
        # 2
        ...
        pool2 = cnn_2.pooling(pool_fun=tf.nn.max_pool, input=bn2) #(-1, 3, 3, 64)
        # 3
        ...
        pool3 = cnn_3.pooling(pool_fun=tf.nn.max_pool, input=bn3) #(-1, 2, 2, 128)
        # flat
...
def train(training_time, is_finishing):
    ...
    with NN_graph.as_default():
        ...
        with tf.name_scope('loss-potimize-evaluation-acc'):
            ...
            acc_fnn = evaluation_fnn.acc_regression(Threshold=0.01)  # 评价指标中的阈值可以修改
            #添加摘要acc
                ...
                for batch_x, batch_y in data_steptwo(train_data=train, batch_size=1000):
                    ...
                    if (epoch % 100) == 0 and flag == 1:
                        loss_fnn_ = sess.run(loss_fnn, feed_dict={x: batch_x[:, :100], x_add: batch_x[:, 100:],
                                                                  ...
                                                                  is_training: False})
                        acc_fnn_, fnn_op_ = sess.run([acc_fnn, fnn_op], feed_dict={x: test[:, :100], x_add: test[:, 100:-1],
                                                                y: test[:, -1][:, np.newaxis],
                                                                is_training: False})
                        acc_divided = evaluation_fnn.acc_regression_divided(Threshold=0.01,
                                                                            divided_point=[0.0, 0.00016, 0.0286, 0.286, 1.0],
                                                                            ndarray_label=test[:, -1],
                                                                            ndarray_logits=fnn_op_.ravel())
                        for key, value in acc_divided.items():
                            print('%s : %s' % (key, value))
